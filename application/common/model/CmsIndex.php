...
class CmsIndex extends Model3 {
    ...
    protected function _after_find(&$result, $options) {
       if ($result['cid']) {
           $result['category_info'] = D2($this->moduleName.'Category')->find($result['cid']);
       }
    }
    ...
    protected function checkPostAuth() {
        if (MODULE_NAME == 'Home') {
            $category_post_auth = D2($this->moduleName.'Category')->getFieldById(I('post.cid'), 'post_auth');
    ...
    public function update() {
        ...
        //调用create方法构造数据
        $cid = I('post.cid');
        $category_info = D2($this->moduleName.'Category')->find($cid);
        $doc_type_info = D2($this->moduleName.'Type')->where(array('id' => $category_info['doc_type']))->find();
        $_POST['doc_type'] = $doc_type_info['id'];
        $base_data = $this->create();
        if ($base_data) {
            //获取当前分类
            $extend_table_object = D2($this->moduleName.ucfirst($doc_type_info['name']));
    ...
    public function getList($cid, $limit = 10, $page = 1, $order = null, $child = false, $map = null) {
        //获取分类信息
        $category_object = D2($this->moduleName.'Category');
        $category_info = $category_object->find($cid);
        $base_table = C('DB_PREFIX').$this->tableName;

        //获取该分类绑定文章模型的主要字段
        $type_object = D2($this->moduleName.'Type');
        ...
        // 拼接文章地址
        $main_field_name = D2($this->moduleName.'Attribute')->getFieldById($type['main_field'], 'name');
    ...
    public function detail($id, $map = null) {
        ...
        // 获取收藏状态
        $info['mark_status'] = D2($this->moduleName.'Mark')->get_mark_status($info['id']);
        ...
        // 获取评论数量
        $info['user']['comment_count'] = D2($this->moduleName.'Comment')->where(array('uid' => $info['uid']))->count();

        // 获取文档模型相关信息
        $doc_type_info = D2($this->moduleName.'Type')->find($info['category_info']['doc_type']);
        ...
        $attribute_object = D2($this->moduleName.'Attribute');
    ...
    private function getPrevious($info) {
        ...
        // 获取扩展信息
        if ($previous) {
            $type = D2($this->moduleName.'Type')->find($previous['doc_type']);
            $main_field_name = D2($this->moduleName.'Attribute')->getFieldById($type['main_field'], 'name');
            $previous['title'] = D2($this->moduleName.ucfirst($type['name']))->getFieldById($previous['id'], $main_field_name);
        }
    ...
    private function getNext($info) {
        ...
        // 获取扩展信息
        if ($next) {
            $type = D2($this->moduleName.'Type')->find($next['doc_type']);
            $main_field_name = D2($this->moduleName.'Attribute')->getFieldById($type['main_field'], 'name');
            $next['title'] = D2($this->moduleName.ucfirst($type['name']))->getFieldById($next['id'], $main_field_name);
        }
