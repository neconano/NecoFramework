 ...
 * 
 */
class CmsAttribute extends Model3 {
    ...
    protected function checkTableExist($doc_type) {
        $table_name = C('DB_PREFIX').$this->moduleName.'_'.D2($this->moduleName.'Type')->getfieldById($doc_type, 'name');
    ...
    public function addField($field) {
        ...
        if ($table_exist) {
            $sql = <<<sql
                ALTER TABLE `__PREFIX__{$this->table_name}`
ADD COLUMN `{$field['name']}` {$field['field']} {$default} COMMENT '{$field['title']}';
sql;
        } else {
        //新建表
        $sql = <<<sql
            CREATE TABLE IF NOT EXISTS `__PREFIX__{$this->table_name}` (
