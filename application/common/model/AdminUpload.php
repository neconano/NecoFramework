 ...
 * 
 */
class AdminUpload extends Model3
{
    /**
     * 数据库表名
     * 
    ...
    protected function _after_find(&$result, $options)
    {
        //获取上传文件的地址
        if (!$result['url']) {
            if (Env::get("url_uls_domain")) {
                $result['url'] = Env::get("url_uls_domain") . $result['path'];
            } else {
    ...
    public function curlUploadFile($url, $filename, $formname = 'file')
    {
		...
		curl_setopt($ch, CURLOPT_COOKIE, $cookielist); //设置cookie
    ...
    public function crop($data = null)
    {
        ...
        // 图片路径
        $data['src'] = UPLOADS_PATH . $data['src'];

        $image->open($data['src']);
        $type = $image->type();
        if ($image) {
            $file = UPLOADS_PATH .'temp/' . \nfutil\Str::randString(12, 1) . '.' . $type;
            $url  = U( request()->module() . "/Upload/upload", array('module_name' => request()->module()), true, true);
    ...
    public function upload($files = null)
    {
        ...
        $dir    = I('dir') ? I('dir') : 'image'; // 上传类型image、flash、media、file
        ...
        $upload                       = new \nfutil\Upload($upload_config, $upload_driver, C("UPLOAD_{$upload_driver}_CONFIG")); // 实例化上传类
        ...
        if (!$info) {
            ...
            if (isset($_GET['temp']) && $_GET['temp'] === 'true') {
                ...
                $upload_data['path'] = '/' . $info['savepath'] . $info['savename'];
                if (isset($info["url"])) {
                    $upload_data['url'] = $info["url"];
                }
