...
class Report extends Admin {
    ...
    public function index() {
        ...
        $data_list = D2('CmsReport')
                   ->page($p, C('ADMIN_PAGE_ROWS'))
                   ->where($map)
                   ->order('id desc')
                   ->select();
        $page = new Page(
            D2('CmsReport')->where($map)->count(),
            C('ADMIN_PAGE_ROWS')
        );
        ...
        $builder->setMetaTitle('举报列表')  // 设置页面标题
                ...
                ->addTableColumn('reason', '举报理由', 'callback', array(D2('CmsReport'), 'reason_list'))
    ...
    public function edit($id) {
        if (IS_POST) {
            ...
            $builder->setMetaTitle('编辑')  // 设置页面标题
                    ...
                    ->addFormItem('reason', 'select', '举报理由', '举报理由', D2('CmsReport')->reason_list())
                    ->addFormItem('abstract', 'textarea', '详情', '详情')
                    ->addFormItem('mobile', 'text', '电话', '电话')
                    ->setFormData(D2('CmsReport')->find($id))
                    ->display();
        }
    }
