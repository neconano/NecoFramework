...
class Type extends Admin {
    ...
    public function index() {
        ...
        $data_list = D2('CmsType')
                   ->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))
                   ->where($map)
                   ->order('sort asc,id asc')
                   ->select();
        $page = new Page(D2('CmsType')->where($map)->count(), C('ADMIN_PAGE_ROWS'));

        $attr['name']  = 'attribute';
        $attr['title'] = '字段管理';
        $attr['class'] = 'label label-success';
        $attr['href']  = U(D2('CmsIndex')->moduleName.'/Attribute/index', array('doc_type' => '__data_id__'));
    ...
    public function edit($id) {
        if (IS_POST) {
            ...
            $document_type_info = D2('CmsType')->find($id);
            ...
            $attribute_list_checkbox = select_list_as_tree('CmsAttribute', $map);
            ...
            $builder->setMetaTitle('编辑模型')   // 设置页面标题
                    ...
                    ->addFormItem('icon', 'icon', '图标', '模型图标')
                    ->addFormItem('sort', 'num', '排序', '用于显示的顺序')
                    ->setFormData(D2('CmsType')->find($id))
                    ->display();
        }
    }
        ...
        switch ($status) {
            case 'delete' :  // 删除条目
                ...
                if ($document_type['system'] === '1') {
                    ...
                    $con['doc_type'] = $ids;
                    $count = D2('CmsCategory')->where($con)->count();
                    if ($count > 0) {
