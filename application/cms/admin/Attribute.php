...
EOF;
    ...
    public function index($doc_type) {
        ...
        $attribute_list = D2('CmsAttribute')
                        ->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))
                        ->order('id desc')
                        ->where($map)
                        ->select();
        $page = new Page(D2('CmsAttribute')->where($map)->count(), C('ADMIN_PAGE_ROWS'));
    ...
    public function add($doc_type) {
        if (IS_POST) {
            ...
            if ($data) {
                ...
                if ($id) {
                    ...
                    if ($result) {
                        ...
                        $document_type_object = D2('CmsType');
            ...
            $builder->setMetaTitle('新增字段')  // 设置页面标题
                    ...
                    ->addFormItem('doc_type', 'select', '文档类型', '文档类型', select_list_as_tree('CmsType'))
    ...
    public function edit($id) {
        if (IS_POST) {
            ...
            $builder->setMetaTitle('编辑字段')  // 设置页面标题
                    ...
                    ->addFormItem('options', 'textarea', '额外选项', '额外选项radio/select等需要配置此项')
                    ->addFormItem('tip', 'textarea', '字段补充说明', '字段补充说明')
                    ->setFormData(D2('CmsAttribute')->find($id))
                    ->setExtraHtml($this->extra_html)
                    ->display();
        }
    }
