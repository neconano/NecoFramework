...
EOF;
    ...
    public function index($group = 1) { 
        ...
        $data_list = D2('CmsCategory')->field('id,pid,group,doc_type,title,url,icon,create_time,sort,status')
                                  ->where($map)->order('sort asc,id asc')->select();

        // 非系统特殊类型则给标题加上链接以便于进入相应文档列表
        foreach ($data_list as &$item) {
            $document_type = D2('CmsType')->find($item['doc_type']);  // 获取当前文档类型
            if ($document_type['system'] === '0') {
                $item['title'] = '<a href="'.U(D2('CmsIndex')->moduleName.'/Index/index', array('cid' => $item['id'])).'">'.$item['title'].'</a>';
            }
        }
        ...
        // 设置Tab导航数据列表
        $category_group_list = D2('CmsCategory')->group_list();  // 获取分类分组
    ...
    public function add($group = 1) {
        if (IS_POST) {
            $category_object = D2('CmsCategory');
            ...
            $category_object = D2('CmsCategory');
            // 获取前台模版供选择
            $template_list = get_template_list('index');
            ...
            $builder->setMetaTitle('新增分类')  // 设置页面标题
                    ...
                    ->addFormItem('group', 'radio', '分组', '分组', D2('CmsCategory')->group_list())
                    ->addFormItem('pid', 'select', '上级分类', '所属的上级分类', select_list_as_tree('CmsCategory', $map, '顶级分类'))
                    ->addFormItem('title', 'text', '分类标题', '分类标题')
                    ->addFormItem('doc_type', 'radio', '分类内容模型', '分类内容模型', select_list_as_tree('CmsType'))
    ...
    public function setStatus($model = CONTROLLER_NAME) {
        ...
        switch ($status) {
            case 'delete' :  // 删除条目
                ...
                $count = D2('CmsIndex')->where($con)->count();
                if ($count == 0) {
