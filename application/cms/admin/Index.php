...
class Index extends Admin {
    ...
    public function index($cid) {
        if ($cid) {
            ...
            //获取该分类绑定文档模型的主要字段
            $type_object = D2('CmsType');
            $article_type = $type_object->find($category_info['doc_type']);
            $article_type_main_field = D2('CmsAttribute')->getFieldById($article_type['main_field'], 'name');
            ...
            $attribute_list_search = D2('CmsAttribute')->where($map)->getField('name', true);
            ...
            //获取文档列表
            $article_list = D2('CmsIndex')->getList($cid, C('ADMIN_PAGE_ROWS'), $_GET['p'] ? : 1, null, false, $map);
            $article_table = strtolower(C('DB_PREFIX').D2('CmsIndex')->moduleName.'_'.$article_type['name']);

            //分页
            $base_table = C('DB_PREFIX').D2('CmsIndex')->tableName;
            $page = new Page(D2('CmsIndex')->where($map)
            ...
            $category_list = D2('CmsCategory')->where($map)->select();
            $tree = new tree();
            ...
            //文档移动POST地址
            $move_url = U(D2('CmsIndex')->moduleName.'/Index/move');
...
EOF;
            ...
            $builder->setMetaTitle($category_info['title']) //设置页面标题
                    ...
                    ->addTopButton('resume', array('model' => D2('CmsIndex')->tableName))  //添加启用按钮
                    ->addTopButton('forbid', array('model' => D2('CmsIndex')->tableName))  //添加禁用按钮
                    ->addTopButton('recycle', array('model' => D2('CmsIndex')->tableName)) //添加回收按钮
    ...
    public function edit($id) {
        //获取文档信息
        $article_info = D2('CmsIndex')->detail($id);
        ...
        if (IS_POST) {
            ...
            //获取当前分类
            $category_info = D2('CmsCategory')->find($article_info['cid']);
            $doc_type = D2('CmsType')->find($category_info['doc_type']);
            $field_sort = json_decode($doc_type['field_sort'], true);
            $field_group = parse_attr($doc_type['field_group']);
    ...
    public function recycle() {
        $map['status'] = array('eq', '-1');
        $article_list = D2('CmsIndex')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))->where($map)->select();
        $page = new Page(D2('CmsIndex')->where($map)->count(), C('ADMIN_PAGE_ROWS'));

        //使用Builder快速建立列表页面。
        $builder = new \nfutil\builder\ListBuilder();
        $builder->setMetaTitle('回收站') //设置页面标题
                ->addTopButton('restore', array('model' => D2('CmsIndex')->tableName)) //添加还原按钮
