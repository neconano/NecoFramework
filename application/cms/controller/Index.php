...
class Index extends Controller {
    ...
    public function index() {
        ...
        if (!$new_doc_type_list) {
            ...
            $con['status'] = 1;
            $con['system'] = 0;
            $doc_type_list = D2('CmsType')->where($con)->select();

            // 获取字段信息
            $new_doc_type_list = array();
            $attribute_object = D2('CmsAttribute');
        ...
        // 显示模板
        $template = get_template('index', 'index');
    ...
    public function detail($id) {
        ...
        // 显示模板
        $template = get_template('index', $info['category_info']['detail_template']);
    ...
    public function lists($cid) {
        // 获取分类信息
        $map['cid'] = $cid;
        $category_info = D2('CmsCategory')->find($cid);
        ...
        $base_table   = C('DB_PREFIX').D2('CmsIndex')->tableName;
        $extend_table = strtolower(C('DB_PREFIX').D2('CmsIndex')->moduleName.'_'.$doc_type_info['name']);
        $document_list = D2('CmsIndex')
                       ...
                       ->select();
        $page = new Page(
            D2('CmsIndex')->where($map)->join($extend_table.' ON '.$base_table.'.id = '.$extend_table.'.id')->count(),
            C('ADMIN_PAGE_ROWS')
        );
        ...
        if (!$document_list) {
            // 获取当前分类的子分类ID列表
            $child_cagegory_id_list = D2('CmsCategory')->where(array('pid' => $cid))->getField('id',true);
        ...
        // 显示模板
        $template = get_template('index', $category_info['index_template']);
    ...
    public function my() {
        ...
        $document_list = D2('CmsIndex')->page(!empty($_GET["p"])?$_GET["p"]:1, C('ADMIN_PAGE_ROWS'))
                                   ->order('sort desc,id desc')
                                   ->where($map)
                                   ->select();
        $page = new Page(D2('CmsIndex')->where($map)->count(), C('ADMIN_PAGE_ROWS'));
        ...
        foreach($document_list as &$document){
            // 合并基础信息与扩展信息
            $doc_type_info = D2('CmsType')->find($document['doc_type']);
            $document = array_merge($document, D(D2('CmsIndex')->moduleName.ucfirst($doc_type_info['name']))->find($document['id']));

            // 给主要字段赋值
            $main_field_name = D2('CmsAttribute')->getFieldById($doc_type_info['main_field'], 'name');
            $document['main_field'] = $document[$main_field_name];

            // 标题带链接
            $val['title_url'] = '<a target="_blank" href="'.U('CmsIndex/detail', array('id' => $document['id'])).'">'.$document['main_field'].'</a>';
        }

        // 使用Builder快速建立列表页面。
        $builder = new \nfutil\builder\ListBuilder();
        $builder->setMetaTitle('我的文档')  // 设置页面标题
                ->addTopButton('addnew', array('href' => U('CmsIndex/add', array('doc_type' => 3))))   // 添加新增按钮
    ...
    public function add() {
        ...
        if (I('get.doc_type')) {
            ...
            $category_info = D2('CmsCategory')->where($map)->order('id asc')->find();
        } elseif (I('get.cid')) {
            $category_info = D2('CmsCategory')->find(I('get.cid'));
        }
        ...
        $doc_type = D2('CmsType')->find($category_info['doc_type']);
        ...
        $attribute_list = D2('CmsAttribute')->where($map)->select();
        ...
        foreach ($attribute_list as $attr) {
            if ($attr['name'] == 'cid') {
                ...
                $attr['options'] = select_list_as_tree('CmsCategory', $con);
            } else {
    ...
    public function edit($id) {
        $this->is_login();
        // 获取文档信息
        $document_info = D2('CmsIndex')->detail($id);

        // 获取当前分类
        $category_info = D2('CmsCategory')->find($document_info['cid']);
        if (!$category_info['post_auth']) {
            $this->error('该分类禁止投稿');
        }
        $doc_type = D2('CmsType')->find($category_info['doc_type']);
    ...
    public function update() {
        $this->is_login();
        $category_info = D2('CmsCategory')->find(I('post.cid'));
        if (!$category_info['post_auth']) {
            $this->error('该分类禁止投稿');
        }
    ...
    public function setStatus($model = CONTROLLER_NAME) {
        ...
        switch ($status) {
            case 'delete' :  // 删除条目
                $map['status'] = -1;
                $info = D2('CmsIndex')->detail($ids, $map);
                $extend_table_object = D(strtolower(D2('CmsIndex')->moduleName.'_'.$info['doc_type_info']['name']));
                ...
                if ($result) {
                    $result2 = D2('CmsIndex')->delete($ids);
                    if ($result2) {
                        $this->success('彻底删除成功');
                    } else {
                        $this->error('删除失败');
