...
$_config = [
    ...
    'user_login_modal'      => APP_DIR . 'user/view/user/login_modal.html',

    'listbuilder_layout'     => BUILDER_DIR . 'listbuilder.html',
    'formbuilder_layout'     => BUILDER_DIR . 'formbuilder.html',
    'stylebuilder_layout'    => BUILDER_DIR . 'style.html',
    'scriptbuilder_layout'   => BUILDER_DIR . 'javascript.html',
    'builder_layout'         => BUILDER_DIR ,
    ...
    // 注册的根命名空间
    'root_namespace'         => [
        'nfutil' => APP_DIR . 'common/util/nfutil/',
    ],
    // 扩展配置文件
    ...
    'template'               => [
        ...
        'taglib_end'      => '>',
        // 预先加载的标签库
        'taglib_pre_load' => '\\app\\common\\taglib\\nf',
    ],
    ...
    'session'                => [
        ...
        'var_session_id' => '',
        // SESSION 前缀
        'prefix'         => strtolower( Env::get("env_pre") . MODULE_MARK . '_'),
        // 驱动方式 支持redis memcache memcached
        'type'           => 'nfutil\session\driver\Mysql', //使用mysql
        'table_name'     => Env::get("session_table"), //指定表
        // 是否自动开启 SESSION
        'auto_start'     => true,
    ],
...
if (is_file(CONF_PATH_COMMON.'db.php')) {
    $db_config = include CONF_PATH_COMMON.'db.php'; // 包含数据库连接配置
}else{
    echo '缺少db.php配置信息：<br>';
    exit;
}
