...
class Addon extends Admin
{
    ...
    public function install()
    {
        ...
        // 安装数据库
        $sql_file = realpath( ADDON_PATH . $addon_name) . '/sql/install.sql';
        ...
        if ($addon_object->add($data)) {
            ...
            $hooks_update = D2('AdminHook')->updateHooks($addon_name);
            if ($hooks_update) {
                S('hooks', null);
                $this->success('安装成功');
            } else {
    ...
    public function uninstall()
    {
        ...
        $hooks_update = D2('AdminHook')->removeHooks($db_addons['name']);
        ...
        // 卸载数据库
        $sql_file = realpath( ADDON_PATH . $db_addons['name']) . '/sql/uninstall.sql';
        if (file_exists($sql_file)) {
            $sql_object = new Sql();
            $sql_status = $sql_object->execute_sql_from_file($sql_file);
            if (!$sql_status) {
    ...
    public function adminList($name, $tab = 1)
    {
        ...
        if ($addon->custom_adminlist) {
            if (strpos($addon->custom_adminlist, "://") > 0) {
                ...
                $this->redirect(addons_url($addon->custom_adminlist),302);
            } else {
                // 自定义一个后台管理默认页面
            ...
            $param = D2('Addons://' . $name . '/' . $admin['model'] . '')->adminList;
            if ($param) {
                // 搜索
    ...
    public function adminAdd($name, $tab)
    {
        ...
        $admin_list         = $addon->admin_list;
        $admin              = $admin_list[$tab];
        $addon_model_object = D2('Addons://' . $name . '/' . $admin['model']);
    ...
    {
        // $model = 'addon://Link/'.ucfirst($model);
        $model = 'Addons://Link/'.ucfirst($model);
