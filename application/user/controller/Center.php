...
class Center extends Controller {
    ...
    public function index() {
        ...
        $user_type_info = D2('UserType')->find($user_info['user_type']);
    ...
    public function nickname() {
        ...
        if (IS_POST) {
            ...
            $builder->setMetaTitle('修改昵称')  // 设置页面标题
                    ...
                    ->setFormData(D2('AdminUser')->detail($uid))
    ...
    public function avatar() {
        ...
        if (IS_POST) {
            if ($_POST) {
                ...
                $result = D2('AdminUpload')->crop($_POST['avatar']);
                if ($result['error'] != 1) {
                    $user_object = D2('AdminUser');
            ...
            $this->assign('user_info', D2('AdminUser')->detail($uid));
    ...
    public function profile() {
        if (IS_POST) {
            ...
            $count = D2('UserAttribute')->where($map)->count();
            if ($count) {
            ...
            $type = $user_info['user_type'];
            $user_type_info = D2('UserType')->find($type);

            // 获取扩展字段
            $map['user_type'] = array('eq', $type);
            $attribute_list[$type] = D2('UserAttribute')->where($map)->order('id asc')->select();
