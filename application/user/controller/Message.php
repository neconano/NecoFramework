...
class Message extends Controller{
    ...
    public function index($type = 0){
        ...
        foreach ($message_type as $key => $val) {
            if ($count = D2('UserMessage')->newMessageCount($key)) {
    ...
    public function newMessageCount($type = null){
        $data['status'] = 1;
        $data['new_message'] = D2('UserMessage')->newMessageCount($type);
    ...
    public function setRead($ids = null, $type = null){
        ...
        $result = D2('UserMessage')->where($map)->setField('is_read', 1);
        if ($result) {
            $this->success('操作成功');
        } else {
            $this->error('操作失败');
