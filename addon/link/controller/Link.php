...
class Link extends Admin
{
    ...
    public function index($type = 1)
    {
        ...
        $builder->setMetaTitle('友情链接列表') // 设置页面标题
            ...
            ->addRightButton('forbid', array('model' => 'link', 'href' => addons_url('Link://Link/edit', array('id' => '__data_id__')))) // 添加禁用/启用按钮
            ->addRightButton('delete', array('model' => 'link')) // 添加删除按钮
            ->display();
    }
    ...
    // {
    //     $map['id'] = I('id');
    //     D('addon://Link/Link')->where($map)->delete();
    //     if ($result) {
